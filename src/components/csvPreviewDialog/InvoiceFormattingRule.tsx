  ...
  const columns = useMemo(() => {
    const cols: ColumnDef<any>[] = [
      ...
      ...headers.map((header) => ({
        ...
        header: header,
        cell: ({ row }: { row: any }) => (
          <div className="w-32 truncate" title={row.original[header]}>
            {row.original[header]}
          </div>
        ),
      })),
    ...
    >
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle>
            Preview Formatting Rules CSV Data: {fileName}
          </DialogTitle>
          ...
          <div className="flex flex-col flex-grow overflow-hidden">
            ...
            {uploadComplete && (
              <Alert className="mb-4 bg-green-50 border-green-500 text-green-800 flex-shrink-0">
                ...
                <AlertDescription>
                  Successfully imported {data.length} formatting rules.
                </AlertDescription>
              </Alert>
            )}
            ...
            {isUploading && (
              <div className="mb-4 space-y-2 flex-shrink-0">
                <div className="flex justify-between text-sm text-gray-500">
                  <span>Uploading {data.length} formatting rules...</span>
            ...
            <div className="mb-4 flex-shrink-0">
              <p className="text-sm text-muted-foreground mb-2">
                Preview of {data.length} formatting rule records
              </p>
            </div>
