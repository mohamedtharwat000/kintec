...
export function CSVPreviewDialog({
  ...
  const columns: ColumnDef<Partial<CalloffWorkOrder>>[] = [
    {
      id: "index",
      header: "#",
    ...
    },
    {
      ...
      header: "Start Date",
      cell: ({ row }) => {
        const date = row.getValue("CWO_start_date");
        return date ? (
    ...
    },
    {
      ...
      header: "End Date",
      cell: ({ row }) => {
        const date = row.getValue("CWO_end_date");
        return date ? (
          <div>{new Date(date as string).toLocaleDateString()}</div>
        ) : (
          <div>-</div>
    ...
    },
    {
      ...
      header: "Total Value",
      cell: ({ row }) => {
        const value = row.getValue("CWO_total_value");
        return value ? (
          <div>
    ...
    >
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle>Call-off Work Order CSV Preview: {fileName}</DialogTitle>
          <DialogDescription>
            Review the call-off work orders before importing. Make sure all
          ...
          <div className="flex flex-col flex-grow overflow-hidden">
            ...
            {uploadComplete && (
              <Alert className="mb-4 bg-green-50 border-green-500 text-green-800 flex-shrink-0">
                ...
                <AlertDescription>
                  Successfully imported {data.length} call-off work orders.
                </AlertDescription>
              </Alert>
            )}
            ...
            {isUploading && (
              <div className="mb-4 space-y-2 flex-shrink-0">
                <div className="flex justify-between text-sm text-gray-500">
                  <span>Uploading {data.length} call-off work orders...</span>
            ...
            <div className="m-4 flex-shrink-0">
              <p className="text-sm text-muted-foreground mb-2">
                Preview of {data.length} call-off work order records
              </p>
            </div>
