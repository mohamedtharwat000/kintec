...
export function CSVPreviewDialog({
  ...
  onConfirm,
  validationErrors = [],
  title = "Preview Submission CSV Data",
  ...
  const columns = useMemo(() => {
    const cols: ColumnDef<any>[] = [
      ...
      ...(headers.includes("contractor_id")
        ...
        : []),
      ...(headers.includes("PO_id") || headers.includes("CWO_id")
        ? [
            {
              id: "order_id",
              header: "Order ID",
      ...
      ...(headers.includes("PO_id") || headers.includes("CWO_id")
        ? [
            {
              ...
              cell: ({ row }: { row: any }) => {
                ...
                return (
                  <div className="w-20">
                    {data.PO_id ? "PO" : data.CWO_id ? "CWO" : "N/A"}
                  </div>
                );
              },
            },
        ...
        : []),
      ...(headers.includes("billing_period")
        ? [
            {
              id: "billing_period",
              ...
              cell: ({ row }: { row: any }) => (
                <div className="w-28">
                  {row.original.billing_period
                    ? format(
                        new Date(row.original.billing_period),
                        "dd/MM/yyyy"
                      )
                    : "N/A"}
                </div>
        ...
        : []),
      ...(headers.includes("payment_currency") &&
      headers.includes("invoice_currency")
        ? [
            {
              id: "currencies",
              header: "Currencies",
              cell: ({ row }: { row: any }) => {
                ...
                return (
                  <div className="w-28">
                    {data.payment_currency} / {data.invoice_currency}
                  </div>
                );
              },
            },
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        <DialogHeader className="flex-shrink-0">
          ...
          <DialogDescription className="text-muted-foreground text-sm">
            Please review the submission data before importing. Ensure all
          ...
          <div className="flex flex-col flex-grow overflow-hidden">
            ...
            {uploadComplete && (
              <Alert className="mb-4 bg-green-50 border-green-500 text-green-800 flex-shrink-0">
                ...
                <AlertDescription>
                  Successfully imported {data.length} submission records.
                </AlertDescription>
              </Alert>
            )}
            ...
            {isUploading && (
              <div className="mb-4 space-y-2 flex-shrink-0">
                <div className="flex justify-between text-sm text-gray-500">
                  <span>Uploading {data.length} submission records...</span>
            ...
            <div className="m-4 flex-shrink-0">
              <p className="text-sm text-muted-foreground mb-2">
                Preview of {data.length} submission records
              </p>
            </div>
