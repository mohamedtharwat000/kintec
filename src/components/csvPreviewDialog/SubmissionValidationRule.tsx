...
export function CSVPreviewDialog({
  ...
  onConfirm,
  validationErrors = [],
  title = "Preview Validation Rules CSV Data",
  ...
  const columns = useMemo(() => {
    const cols: ColumnDef<any>[] = [
      ...
      ...(headers.includes("submission_id")
        ...
        : []),
      ...(headers.includes("required_fields")
        ? [
            {
              id: "required_fields",
              ...
              cell: ({ row }: { row: any }) => (
                <div className="w-28">
                  {row.original.required_fields || "N/A"}
                </div>
              ),
            },
          ]
        : []),
      ...(headers.includes("approval_signature_rules")
        ? [
            {
              id: "approval_signature_rules",
              accessorKey: "approval_signature_rules",
              header: "Signature Rules",
              cell: ({ row }: { row: any }) => (
                <div className="w-40 truncate">
                  {row.original.approval_signature_rules || "N/A"}
                </div>
              ),
            },
          ]
        : []),
      ...(headers.includes("approval_date_rules")
        ? [
            {
              id: "approval_date_rules",
              accessorKey: "approval_date_rules",
              header: "Date Rules",
              cell: ({ row }: { row: any }) => (
                <div className="w-40 truncate">
                  {row.original.approval_date_rules || "N/A"}
                </div>
              ),
            },
          ]
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        <DialogHeader className="flex-shrink-0">
          ...
          <DialogDescription className="text-muted-foreground text-sm">
            Please review the validation rule data before importing. Ensure all
          ...
          <div className="flex flex-col flex-grow overflow-hidden">
            ...
            {uploadComplete && (
              <Alert className="mb-4 bg-green-50 border-green-500 text-green-800 flex-shrink-0">
                ...
                <AlertDescription>
                  Successfully imported {data.length} validation rule records.
                </AlertDescription>
              </Alert>
            )}
            ...
            {isUploading && (
              <div className="mb-4 space-y-2 flex-shrink-0">
                <div className="flex justify-between text-sm text-gray-500">
                  <span>
                    Uploading {data.length} validation rule records...
                  </span>
            ...
            <div className="m-4 flex-shrink-0">
              <p className="text-sm text-muted-foreground mb-2">
                Preview of {data.length} validation rule records
              </p>
            </div>
